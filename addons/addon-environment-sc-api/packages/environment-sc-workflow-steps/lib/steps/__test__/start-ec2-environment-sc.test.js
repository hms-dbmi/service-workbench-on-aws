...
describe('StartEc2EnvironmentStep', () => {
  ...
  const meta = { workflowId: `wf-start-ec2-environment-sc` };
  const input = {
  ...
  const step = new StartEc2EnvironmentSc({
    step: { config: {} },
    workflowPayload: new WorkflowPayload({ meta, input, workflowInstance: { steps: ['st-start-ec2-environment-sc'] } }),
  });
  let ec2;
